...
describe("signupRoles", () => {
  ...
  it("returns the roles unwrapped so the customer-role _id lookup keeps working", async () => {
    mockRequest.mockResolvedValueOnce({ signupRoles: LIVE_SIGNUP_ROLES });

    const roles = await signupRoles();

    expect(roles).toEqual(LIVE_SIGNUP_ROLES);
    // This is the exact lookup app/(auth)/complete.tsx performs before register.
    expect(roles.find((r) => r.roleId === "customer")?._id).toBe(
      "6a11bcb8ffd7d2160b1e53bd",
    );
  });
});
