...
// Optional packages that may not be installed yet.
// Metro falls back to local stubs so bundling succeeds without the real packages.
const OPTIONAL_STUBS = {
  "expo-document-picker": path.join(projectRoot, "src/stubs/expo-document-picker.js"),
  "expo-image-picker":    path.join(projectRoot, "src/stubs/expo-image-picker.js"),
};
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  if (OPTIONAL_STUBS[moduleName]) {
    try {
      return context.resolveRequest(context, moduleName, platform);
    } catch (_e) {
      return { filePath: OPTIONAL_STUBS[moduleName], type: "sourceFile" };
    }
  }
